...
// Math utility functions from Hogent
function mod(value: number, modulus: number): number {
  return ((value % modulus) + modulus) % modulus;
}

function modRad(value: number): number {
  while (value > Math.PI) value -= 2 * Math.PI;
  while (value < -Math.PI) value += 2 * Math.PI;
  return value;
}

export function useGyrAngle(gyr: { x: number; y: number; z: number }) {
...
export function useAttitude(acc: { x: number; y: number; z: number }, mag: { x: number; y: number; z: number }, gyr: { x: number; y: number; z: number }) {
  // Constructor
...
export function useAccStep(acc: { x: number; y: number; z: number }, mag: { x: number; y: number; z: number }, gyr: { x: number; y: number; z: number }) {
  // Constructor
  const acc_inv = object_sign_inversion(acc);

  // States
  const [gravity, setGravity] = useState({ x: 0, y: 0, z: 1 });
  const [movingWindow, setMovingWindow] = useState<number[]>([]);
...
export function useHeading(acc: { x: number; y: number; z: number }, mag: { x: number; y: number; z: number }, gyr: { x: number; y: number; z: number }) {
  // Constructor
  const acc_inv = object_sign_inversion(acc);

  // States
  const [gravity, setGravity] = useState({ x: 0, y: 0, z: 9.81 });
  const [gyrAngTI, setGyrAngTI] = useState<string[]>([]);
  ...
  const _algorithm = (h_mag: number, h_gyr: number, h_mag_prev: number | null, h_t_prev: number) => {
    ...
    let diff = {
      ...
      h_mag_diff: h_mag_prev ? Math.abs(h_mag - h_mag_prev) : 0,
    };
    let h_t = 0;
...
/**
 * Returns compass heading as rads - EXACT Hogent implementation
 */
...
export function useStepLength(acc: { x: number; y: number; z: number }, mag: { x: number; y: number; z: number }, gyr: { x: number; y: number; z: number }, steplengthScaler = 1, offset?: number) {
  // Custom Hooks - EXACT Hogent implementation
  const [accStep, accEvent] = useAccStep(acc, mag, gyr);
  const heading = useHeading(acc, mag, gyr, offset);

  // States
  const [accList, setAccList] = useState<number[]>([]);
  const [headingList, setHeadingList] = useState<number[]>([]);
  const [valleyList, setValleyList] = useState<number[]>([]);
