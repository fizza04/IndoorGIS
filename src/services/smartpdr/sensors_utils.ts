export function object_sign_inversion(obj: { x: number; y: number; z: number }) {
...
export function LPFilter(value1: number, value2: number, alpha = 0.95) {
  return alpha * value1 + (1 - alpha) * value2;
}

export function compFilter(value1: number, value2: number, alpha = 0.95) {
  return Math.abs(value1 - value2) < Math.PI
    ? alpha * value1 + (1 - alpha) * value2
    : value2;
}

export function argmin(list1: number[], list2 = list1) {
  return list2.indexOf(list1.reduce((a, b) => Math.min(a, b)));
}

export function toGCS(lcs: { x: number; y: number; z: number }, attitude: { pitch: number; roll: number; yaw: number }, T = false) {
  ...
  const _matrix_times = (mat1: number[][], mat2: number[][]) => {
    let ret = new Array();
    for (let i = 0; i < 3; i++) {
